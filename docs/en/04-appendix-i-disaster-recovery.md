...
Level 1	Partial service degradation (1-2 containers)	≤ 15 minutes	0 minutes (hot standby)
Level 2	Loss of one critical service (DB, MinIO, Elasticsearch)	≤ 1 hour	5 minutes (replication)
Level 3	Complete system loss (host failure, datacenter outage)	≤ 4 hours	1 hour (hourly snapshots)
Level 4	Catastrophic failure (data loss, compromise)	≤ 24 hours	24 hours (daily backups)
...
echo "🔧 Level 1 Recovery: Partial Degradation"
...
for SERVICE in $FAILED_SERVICES; do
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "❌ $SERVICE not recovered, escalating to level 2"
...
echo "🚨 Level 2 Recovery: Loss of $SERVICE"
...
echo "💥 Level 3 Recovery: $DISASTER_TYPE"
...
case $DISASTER_TYPE in
    "host_failure")
        echo "🖥️  Recovering from host failure..."
    ...
    "datacenter_outage")
        echo "🏢 Recovering from datacenter outage..."
    ...
    "security_breach")
        echo "🔒 Recovering from security breach..."
...
# 4. Start system
echo "🚀 Starting recovered system..."
docker compose up -d
...
echo "✅ Hourly snapshot created: $BACKUP_DIR"
...
echo "✅ Daily backup created: $BACKUP_DIR"
